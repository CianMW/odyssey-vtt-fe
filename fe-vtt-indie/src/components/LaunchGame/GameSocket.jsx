...
const GameSocket = () => {
  ...
  const joinRoom = (room) => {
    socket.emit("joinRoom", ({gameId: room, username: currentState.user.info.name }))
  }
  
  // const ADDRESS = process.env // <-- address of the BACKEND PROCESS
  ...
  return (
    <Container className=" m-0 p-0" >
      ...
      <Col md={9} className=" static-full-height-scroll px-2 m-0 col-md-8 bg-custom-textured">
        <CharacterSheet/>
      </Col>
      <Col md={3} className="px-0 m-0 full-height col-md-1">
        <Row className="">
           <Col md={3} className={`text-center dynamic-folder ${selectedFolder === "chat" ? "semi-bordered":"bordered"}`} onClick={e => setSelectedFolder("chat")}><i className="bi bi-chat-left-text-fill"></i></Col>
           <Col md={3} className={`text-center dynamic-folder ${selectedFolder === "characters" ? "semi-bordered":"bordered"}`} onClick={e => setSelectedFolder("characters")}><i className="bi bi-folder-fill"></i></Col>
           <Col md={3} className={`text-center dynamic-folder ${selectedFolder === "library" ? "semi-bordered":"bordered"}`} onClick={e => setSelectedFolder("library")}><i className="bi bi-search"></i></Col>
           <Col md={3} className={`text-center dynamic-folder ${selectedFolder === "settings" ? "semi-bordered":"bordered"}`} onClick={e => setSelectedFolder("settings")}><i className="bi bi-gear-fill"></i></Col>
        </Row>
